...
**Status:** Accuracy foundation (task 2) verified 2026-09-18; lab aimed attack (task 3) and HUD aim selection (task 4) and line-of-fire/exposure (task 5), physical visibility (task 6), bounded location injury (task 7), throat/vocal integration (task 8), persistent serious injuries (task 9), the atomic treatment operation (task 10A), the healer service interaction (task 10B), qualified field treatment (task 10C) implemented 2026-09-19; production anatomy and aim content (task 11, torso/arm/throat slice) and enemy called-shot AI (task 12) implemented 2026-09-20; Checkpoints B, Recovery, and D (automated parts) reached. Legs slice (minor hobble through the active scheduler) implemented 2026-09-20. Head/eyes slice (minor sight term on ranged and aimed shots) implemented 2026-09-20. Serious production injuries (authored escalation thresholds, treatment on all four regions) implemented 2026-09-20. Task 11 content complete; balance pass measured and applied 2026-09-20 (party anatomy default, serious thresholds 9/10). The human tactical judgment and the owner decisions in the balance doc remain open.
...
**2026-09-19 progress:** Task 3 landed: `CalledShot` legality/pricing, controller aim query at forecast and commit, lab fixture with synthetic anatomy, and 9 aimed-action tests under both schedulers (`test/integration/test_called_shots.gd`, `test/unit/test_combat_lab.gd`). A JSON-replayed aim context now logs identically to the live one. The lab panel was not inspected rendered; Checkpoint A's third item is covered only by the hidden-result assertions from task 2. Task 4 landed the same day: forecast-panel aim row, interface aim mode, controller-quoted disabled reasons, rendered capture. [Evidence](../docs/qa/called-shot-hud-2026-09-19.md). Task 5 landed: authored obstacles refuse `blocked_by_obstacle`, low cover hides cover-hidden anatomy (`aim_cover`) unless seen over from height, legacy cover mitigation retained without a second charge. [Evidence](../docs/qa/called-shot-exposure-2026-09-19.md). Task 6 landed: per-cell clear/dim/obscured composed worst-of, ranged-only applicability, Blinded reconciled to its facing restriction, provisional −10/−25 pp, live forecast refresh. [Evidence](../docs/qa/called-shot-visibility-2026-09-19.md). Task 7 landed: `CombatInjury` records on `BattleActor.injuries` (one per location, refresh on repeat, replay no-op), injury eligibility decided after mitigation, arm injury as a named −10 pp attack modifier that leaves Alacrity and spells alone, HUD/lab quotes with on-hit and overall chance. [Evidence](../docs/qa/called-shot-injury-2026-09-19.md). Task 8 landed: `CombatAction.requires_voice` delivery metadata, minor throat as a voice-only accuracy term, severe throat (`voice_blocked`) refusing `voice_required` before payment, and a voice-blocking hit releasing the target's held Note once through `release_hold`. Muted is untouched. [Evidence](../docs/qa/called-shot-vocal-2026-09-19.md). Task 9 landed: `PartyMember.injuries` serialized as an optional key (old saves load empty, malformed records dropped, unknown record keys kept), mirrored into `BattleActor`, written back by `Battle` at victory/defeat/flee with minor records dropped; hostile records live on the Hostile node's actor for the field's lifetime. No schema bump. [Evidence](../docs/qa/called-shot-persistence-2026-09-19.md). Task 10A landed: `InjuryTreatment` quote/commit coordinator over `PartyMember.injuries`, `spend_gp`, and `remove_items`; records carry `instance_id` + revision; test cards (20 GP service, one-supply field with Trained Mending); dev-console `treat` fixture. [Evidence](../docs/qa/called-shot-treatment-2026-09-19.md). Task 10B landed (owner chose providers 2026-09-19): Root & Reed herbalist sets wounds for a PROVISIONAL 20 GP through a TREATMENT section on the shop screen; the Shrine of the Held Flame gives one succor per game regardless of standing or purse (flag `dom_shrine_succor_spent`) as the authored no-cash/locked-out route; disabled rows name the reason and the alternative. [Evidence](../docs/qa/called-shot-healer-2026-09-19.md). Task 10C landed: `field-mending` card (Trained Mending, one Bitterleaf Poultice as the PROVISIONAL supply used as-is, practitioner needs an unhurt arm) on an Injuries section of the character sheet with a practitioner pick that defaults to the first qualified member; same quote/commit path as the shop; combat start between quote and press refuses without spending. [Evidence](../docs/qa/called-shot-field-treatment-2026-09-19.md). 

**2026-09-20 progress:** Task 11 landed for the torso/arm/throat slice: canon archetypes author `anatomy`, validated by the canon reader, seeded as a Pandora `Anatomy` column, exported into `encounters.json`, and placed on built actors; `strike` and `enemy-strike` author PROVISIONAL aim profiles with minor injuries and explicit CT costs. Task 12 landed: `choose_enemy_aim` scores bounded legal aims from forecast data only. Checkpoint D: unit/resolution/integration suites green, drift check clean, ~2.5 ms per enemy decision on a 100-actor grid (i5-13400F). [Evidence](../docs/qa/called-shot-content-and-ai-2026-09-20.md). Legs slice landed the same day: `move_cost_percent` injury effect priced through both schedulers for players and enemies, `leg` anatomy on the five humanoid/hound archetypes, PROVISIONAL `leg` profile on both strikes, rendered aim and hobbled-move evidence. [Evidence](../docs/qa/called-shot-legs-2026-09-20.md). Head/eyes slice landed: `sight_accuracy_pp` on ranged non-spell and every aimed shot, `head` anatomy (guard helm unexposed), PROVISIONAL `head` profile on both strikes, rendered evidence. [Evidence](../docs/qa/called-shot-head-2026-09-20.md). Serious slice landed: optional `serious` variant per aim injury with a higher damage threshold, applied on the same roll; HUD `· SERIOUS` marker; provisional arm/leg/head/throat variants on both strikes; every treatment card cures all four regions. [Evidence](../docs/qa/called-shot-serious-2026-09-20.md). Balance pass: probe over the starting party × six archetypes found party members had no anatomy (enemy AI and party persistence unreachable) and serious thresholds above any reachable damage; fixed with a PROVISIONAL humanoid `PartyMember.anatomy` default and thresholds 9 (arm/leg) / 10 (head/throat). [Evidence and owner decisions](../docs/qa/called-shot-balance-2026-09-20.md). Open: human tactical judgment, enemy damage vs party (1–4), per-location AI value, a voice-tagged production action.
...
**Acceptance:**
- [x] Player selects an injury, sees an exact GP quote, commits through the interaction, and immediately regains the corresponding permitted actions.
- [x] Provider supplies are included in the fee; lack of party Mending does not block the route, and invalid/changed quotes cannot charge silently.
- [x] Encounter has a verified recovery path for no cash, no party Mending, and an injured practitioner; author the access solution without a new global free-healing mechanic.
...
- [x] A party without Mending can complete the persistent injury/recovery loop.
- [x] Transaction, save, and rendered interaction checks pass; neither failure nor duplicate submission loses resources.
- [x] A guaranteed treatment consumes no SkillCheck RNG or Expert reroll.
...
**Acceptance:**
- [x] A qualified, conscious practitioner can cure one supported injury outside combat using the displayed supply quantity; ineligible self-treatment gives a precise reason.
- [x] Field treatment uses the same quote/commit/notification rules as provider treatment and rejects if combat begins before commitment.
- [x] Cure removes only the selected injury; HP, other injuries, elemental effects, Soul, and ultimate flags follow their independent contracts.
...
**Acceptance:**
- [x] Humanoid and non-humanoid profiles validate stable IDs, absent parts, delivery requirements, costs, and outcome references.
- [x] Production catalogs regenerate reproducibly; no generated artifact is hand-edited.
- [x] Each newly enabled region has a useful bounded effect, accurate forecast, and serious-injury persistence/recovery consistent with the confirmed rule.
...
**Acceptance:**
- [x] AI can prefer a useful throat/arm shot and still choose ordinary damage when its expected value is better.
- [x] AI sees only observable anatomy/conditions and known weaknesses; it never reads future rolls.
- [x] Candidate generation is bounded and respects injury restrictions, costs, and legal target exposure.
...
- [x] Run affected unit/integration suites and inspect the rendered combat encounter; fix regressions caused by this work.
- [x] Compare ordinary-versus-aimed decisions, save compatibility, both schedulers, and replay against the accepted contracts.
