...
**2026-09-20 progress:** Task 11 landed for the torso/arm/throat slice: canon archetypes author `anatomy`, validated by the canon reader, seeded as a Pandora `Anatomy` column, exported into `encounters.json`, and placed on built actors; `strike` and `enemy-strike` author PROVISIONAL aim profiles with minor injuries and explicit CT costs. Task 12 landed: `choose_enemy_aim` scores bounded legal aims from forecast data only. Checkpoint D: unit/resolution/integration suites green, drift check clean, ~2.5 ms per enemy decision on a 100-actor grid (i5-13400F). [Evidence](../docs/qa/called-shot-content-and-ai-2026-09-20.md). Open: legs and head/eyes slices, serious production injuries, human tactical judgment.
