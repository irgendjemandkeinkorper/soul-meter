...
func test_quest_button_is_available_for_each_new_companion() -> void:
	var vex := PartyMember.new()
	vex.id = GameState.PROTAGONIST_ID
	vex.display_name = "Vex"
	for companion_id: String in PackedStringArray(
		["ressa-quickfingers", "korrath-ninefold", "maura-greyfen"]
	):
		var companion: PartyMember = null
		for candidate: PartyMember in GameState.recruitable_candidates():
			if candidate.id == companion_id:
				companion = candidate
				break
		assert_object(companion).is_not_null()
		GameState.party = [vex, companion]
		...
		assert_object(button).is_not_null()
		assert_bool(button.visible).is_true()
		assert_str(button.text).contains(companion.display_name)
