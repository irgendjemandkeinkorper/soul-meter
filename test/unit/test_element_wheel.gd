...
func test_normalize_index_and_circular_distance() -> void:
	assert_str(String(ElementWheel.normalize(" Sul "))).is_equal(" sul ")
	assert_int(ElementWheel.index_of(&"sul")).is_equal(0)
	assert_int(ElementWheel.distance(&"sul", &"zhur")).is_equal(1)
	assert_int(ElementWheel.distance(&"unknown", &"sul")).is_equal(-1)

func test_span_rejects_unknown_or_duplicate_and_finds_outer_gap() -> void:
	assert_int(ElementWheel.span([])).is_equal(0)
	assert_int(ElementWheel.span([&"sul", &"sul"])).is_equal(-1)
	assert_int(ElementWheel.span([&"sul", &"luth", &"tham"])).is_equal(4)
	assert_int(ElementWheel.span([&"missing", &"luth"])).is_equal(-1)

func test_contains_opposed_pair_only_for_diametric_elements() -> void:
	assert_bool(ElementWheel.contains_opposed_pair([&"sul", &"vekh"])).is_true()
	assert_bool(ElementWheel.contains_opposed_pair([&"sul", &"vel"])).is_false()

func test_opposite_returns_the_element_at_wheel_distance_five() -> void:
	assert_str(String(ElementWheel.opposite(&"sul"))).is_equal("vekh")
	assert_str(String(ElementWheel.opposite(&"vekh"))).is_equal("sul")
	assert_int(ElementWheel.distance(&"zhur", ElementWheel.opposite(&"zhur"))).is_equal(5)
