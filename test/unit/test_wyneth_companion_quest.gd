...
## FR-505 companion personal quests — the second worked example (Wyneth Hallow-Tide).
## Mirrors test_companion_quest.gd's Serai-Lun coverage; see that file and
## CLAUDE.md for the ratified FR-505 scope decision.

const DIALOGUE_PATH := "res://dialogue/companions/wyneth_hallow_tide.dialogue"
...
func _wyneth() -> PartyMember:
	for candidate in GameState.recruitable_candidates():
		if candidate.id == "wyneth-hallow-tide":
			return candidate
	fail("wyneth-hallow-tide is no longer a recruitable candidate")
	return null
...
func test_joining_the_party_offers_her_personal_quest_exactly_once() -> void:
	assert_bool(QuestRegistry.is_active(QuestRegistry.WYNETH_QUEST)).is_false()
	assert_bool(GameState.set_companions([_wyneth(), _other_recruit()])).is_true()
	assert_bool(QuestRegistry.is_active(QuestRegistry.WYNETH_QUEST)).is_true()
