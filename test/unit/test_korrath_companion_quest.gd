...
## FR-505 companion personal quest coverage for Korrath Ninefold.
...
const COMPANION_ID := "korrath-ninefold"
const DIALOGUE_PATH := "res://dialogue/companions/korrath_ninefold.dialogue"
...
func test_joining_party_offers_korrath_quest_and_dialogue_is_loadable() -> void:
	var korrath: PartyMember = _candidate(COMPANION_ID)
	var other: PartyMember = _other_recruit(COMPANION_ID)
	_unlock_recruit(korrath)
	_unlock_recruit(other)

	assert_bool(GameState.set_companions([korrath, other])).is_true()
	assert_bool(QuestRegistry.is_active(QuestRegistry.KORRATH_QUEST)).is_true()
