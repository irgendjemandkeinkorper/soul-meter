...
## FR-505 companion personal quest coverage for Ressa Quickfingers.
...
const COMPANION_ID := "ressa-quickfingers"
const DIALOGUE_PATH := "res://dialogue/companions/ressa_quickfingers.dialogue"
...
func test_joining_party_offers_ressa_quest_and_dialogue_is_loadable() -> void:
	var ressa: PartyMember = _candidate(COMPANION_ID)
	var other: PartyMember = _other_recruit(COMPANION_ID)
	_unlock_recruit(ressa)
	_unlock_recruit(other)

	assert_bool(GameState.set_companions([ressa, other])).is_true()
	assert_bool(QuestRegistry.is_active(QuestRegistry.RESSA_QUEST)).is_true()
