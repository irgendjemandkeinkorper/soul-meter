...
func test_attack_summary_explains_target_balance_and_cost() -> void:
	...
	assert_str(action.summary()).is_equal("Enemy target · +25 Order · 1 AP + 2 Soul")
...
func test_defensive_summaries_explain_self_target_and_balance_effect() -> void:
	...
	assert_str(guard.summary()).is_equal("Self · Reduces the next hit · 1 AP")
	assert_str(stabilize.summary()).is_equal(
		"Self · Pulls Balance 30 toward Equilibrium · 1 AP"
	)
...
func test_resolution_summary_explains_that_it_ends_the_encounter() -> void:
	...
	assert_str(action.summary()).is_equal("Encounter · Ends the encounter · 0 AP")
