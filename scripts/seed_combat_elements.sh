...
# Applies the idempotent chapter-one Pandora migration (tools/seed_chapter_one.gd — now
# carrying combatant Element Id), then regenerates every Pandora-derived artifact through
# the project's canonical generator. Mirrors scripts/seed_town_npcs.sh's run_tool pattern.
...
run_tool \
	"ChapterOneSeed" \
	"res://tools/seed_chapter_one.gd" \
	"CHAPTER-SEED: chapter-one entities present\."
run_tool \
	"ElementGenerator" \
