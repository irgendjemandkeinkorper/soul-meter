...
## 2026-08-04 - [Missing Length Bounds in Stable ID Validation]
**Vulnerability:** While `StableIds.is_valid()` validates the format of identifiers using regex, it does not enforce any length bounds. This meant that keys in `zhavar` or `var_harmony` could contain multi-megabyte string payloads that were considered "valid" identifiers, exposing the game's deserialization pipeline to memory exhaustion DoS when loading untrusted saves.
**Learning:** Opaque identifier validation (such as regex checking for character sets) is not a substitute for quantitative length limiting. Deserializing dictionaries from user-controlled files must explicitly enforce length constraints on both keys and values, even when validating them against regex-based format checkers.
**Prevention:** Always pair `StableIds.is_valid()` with an explicit length boundary check (e.g. `(id as String).length() <= 64`) before loading them into active collections.
