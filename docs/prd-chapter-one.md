...
- **FR-103 (P0)** **Called shots and Defining Strikes.** User amendment, 2026-09-18: exposed ordinary anatomy is targetable without discovery or a Lore roll, through an aim-location selector; throat replaces groin. Hidden or supernatural weaknesses retain discovery and the existing Defining Strike knowledge check. A successful knowledge check alone cannot apply a targeted effect: the physical strike must also hit. Existing Defining Strike costs remain; new anatomical aim costs, penalties, and injury outcomes await authoring and tuning under the [combat expansion design](ideas/called-shots-and-injuries.md). This amendment supersedes the earlier restriction on body-part menus. Ordinary anatomy selection is planned; the current runtime still uses the discovered-weakness interface.
...
- **FR-302 (P0)** **Ten elements, each with fixed Imposition (status) + Rule-Bend:** Sul (Exposed / reveals Aftertones, signatures, illusions), Vel (Overgrown / extends own buff duration), Luth (Soaked / only element restoring Breath), Khor (— / *adds*: extends durations, holds Notes across rounds), Tham (Weighted / creates cover, anchors Aftertones), Vekh (Blinded / conceals Discord signatures), Mozh (Decaying / corpses & objects → Breath), Khash (Burning / consumes Aftertone for burst power), Zhem (Muted / *subtracts*: cancels buffs, ends Aftertones, zeroes Tempo), Zhur (Shocked / ignores Instability die). Khor and Zhem deal no damage.
...
- **FR-304 (P0)** **The ten Triads** (span → center): Dayspring (Zhur–Sul–Vel → Sul), Fruiting (Sul–Vel–Luth → Vel), Rivermouth (Vel–Luth–Khor → Luth), Founding (Luth–Khor–Tham → Khor), Vault (Khor–Tham–Vekh → Tham), Barrow (Tham–Vekh–Mozh → Vekh), Pyre (Vekh–Mozh–Khash → Mozh), Cinderfall (Mozh–Khash–Zhem → Khash), Stillpoint (Khash–Zhem–Zhur → Zhem), Thunderhead (Zhem–Zhur–Sul → Zhur). Every element rules exactly one Triad and wings two. Each Triad's unique effect: ⚑ ten designs needed, propose in Phase 2, ratify to vault.
...
- **FR-308 (P1)** **The Zhavar** (zone-scale, replaces "Carry"/"Overtone" ⚑ vault rename): banking harmony raises local Integrity AND the Zhavar (how far the zone "can be heard"). Escalation ladder: low → rising → tolling (one Zhem dragon) → ringing (regional delegation) → unprecedented. In Chapter 1: Zhavar is tracked and *telegraphed* (rumors, ambient VFX, one scripted tolling event); full dragon-response systemization is Chapter 2+.
- **FR-309 (P1)** Casting economy integration: Gauge/Breath split, Aftertones, Tempo per the Ledger expansion points ⚑ — stat the minimum needed for FR-302's Rule-Bends to function (Aftertones and Breath are load-bearing: Tham anchors them, Khash consumes them, Mozh/Luth restore Breath, Zhem zeroes Tempo).
...
| **Fizzle/percentile double-randomness frustration** (d100 skill + fizzle + Instability die) | M | M | One roll surface per action where possible; Zhur's Rule-Bend and Masteries exist to delete dice; show the math (FR-205) |
