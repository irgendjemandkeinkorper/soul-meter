...
| **SOFT-FAIL** | Playable, but thematically incoherent. Ships without support content — no dedicated art, barks, or ability variants. |
| **HARD-FAIL** | Mechanical contradiction: two abilities that cannot both resolve, or a Discipline that displaces a Patron's signature loop. **Triggers the abort criterion** — stop and use direct mappings instead. |

Sources: `~/projects/dramgid-vault/systems/ten-patron-classes.md` (Kit / Resource / Signature per
patron) and the Combat Disciplines section added to the same file 2026-08-05.
...
## Chordblade — short-reach, high-mobility melee, Khor tempo

| Patron | Verdict | Reasoning |
|---|---|---|
| Mirrorblade | PASS | Paired daggers + Mirrorwalking footwork *is* this profile. Balance alternation (strike/guard) maps directly onto tempo. The archetypal pairing. |
| River-Mother | PASS | Net-and-whip is mid-reach; mobility helps reach downed allies for Last Washing. The Name-Ledger has no positional requirement. |
| Ironbrand | PASS | Greatsword on a short-reach discipline is friction, not contradiction. Vanguard footwork puts you where Scars accumulate. |
| Lensbearer | PASS | Quarterstaff fits. Fading and Sacred Clarity are information effects, position-independent. |
| Husk-bearer | PASS | Cleaver is short. Mobility spreads DoTs, which is exactly what The Table rewards. |
| **Flamebinder** | **SOFT-FAIL** | Deployable kinetic-sculpture constructs are static emplacements. A high-mobility vanguard discipline pulls you away from your own turret. Playable; incoherent. |
...
| Patron | Verdict | Reasoning |
|---|---|---|
| Mirrorblade | PASS | Mild tension — a duelist who plants — but Balance alternation is untouched. |
| River-Mother | PASS | Support holding a raised position is coherent. |
| Ironbrand | PASS (strong) | Holds ground, takes hits, banks Scars. Elevation advantage stacks with the greatsword. |
...
| Patron | Verdict | Reasoning |
|---|---|---|
| Mirrorblade | PASS (note) | The vault says a Mirror Choir build "literally cannot take both music and silence" — but that binds **elements**, not Discipline. No mechanical bar. Worth a line in any player-facing text, because it is easy to misread. |
| **River-Mother** | **SOFT-FAIL** | Kit is a **water harp**; the Name-Ledger recovers Gauge by *naming* the dead and saved. A silence Discipline sits against the class's entire recovery lever. |
| Ironbrand | PASS | Scars are physical; silence is irrelevant to them. Flavourless but sound. |
| Lensbearer | PASS | Revelation plus denial is a coherent lockdown build. |
