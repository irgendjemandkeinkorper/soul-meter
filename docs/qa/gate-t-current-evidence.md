...
| 10 | CT→AP promotion completeness | Automated PASS (2026-08-28) | AP is the shipped default for every encounter (`encounter_catalog.gd` CT overrides removed); balance evidence `docs/qa/wave1-ap-balance-evidence.md`; acceptance evidence incl. screenshot + sweep hashes `docs/qa/wave1-acceptance-evidence.md`; CT scheduler-compatibility tests still pass (`test_deployment_flow.gd`, `test_wavec_tactical_gates.gd`). Historical baseline preserved: Wave-C compatibility characterization; ticket #176's shim-deletion plan is superseded by the promotion. |
