...
3. **No combination grants Soul.** Luth replenishes Breath directly; Mozh converts finite
   corpses or objects into Breath. Haeren and Vhorr retain their separate class Breath refunds.
   Element or class recovery must not create a repeatable infinite-resource cycle.
4. Differences must create decisions: a deadline to prepare for, a condition to avoid, an
   information advantage, a resource to preserve, or terrain to exploit. Names alone do not qualify.
5. Terrain effects follow the world's persistence rules. Destruction remains after combat;
   rebuilding belongs mainly to maintained infrastructure. Temporary summoned cover needs a
   defined expiry and cannot silently restore previously destroyed structures.

## Element key

| Element | Identity | Existing mechanical anchor |
|---|---|---|
...
| Khash | Fire / burning | Burning; consumes an Aftertone for burst power |
| Zhem | Silence / ending | Muted; cancels buffs, ends Aftertones, zeroes Tempo; no direct damage |
| Zhur | Storm / lightning | Shocked; ignores the Instability die, not every source of fizzle |
...
**Divine engine:** Record living allies; keep them in the fight and receive Breath for their fall or survival through victory.
...
| Khash | **Ash Mother** | Record allies and use controlled fire to keep pursuers away; consume Aftertones for a rescue-opening burst while avoiding the refuge's flammable structures. |
| Zhem | **Quiet Witness** | Record threatened allies, then cancel enemy buffs and sustained effects that endanger them; protect the party through subtraction rather than elemental damage. |
| Zhur | **Raincaller** | Record the ally holding the line and use shock pressure to interrupt an enemy advance; preserve the remembered party while the storm disrupts its attackers. |

### Kero — Ironbrand

**Divine engine:** Survive HP loss to bank Scars, then spend them on a decisive guaranteed-hit window.
...
| Khash | **Cinderbrand** | Bank Scars through close combat, then guarantee the hit of a fire technique and consume an Aftertone for burst power; fizzle remains a separate risk. |
| Zhem | **Unbroken** | Strip an enemy's buffs with silence, then spend Scars on a mundane counterattack; remove the protection that would blunt your earned hit. |
| Zhur | **Thunderbrand** | Bank Scars while contesting a chokepoint, then guarantee a lightning hit; shock controls the exchange without relying on the Instability die. |

### Stuid — Lensbearer

**Divine engine:** Spend scarce Clarity to expose information, then exploit what the forecast tells you.
...
| Khash | **Emberseer** | Preview the true outcome before consuming an Aftertone for fire burst; choose the moment when the expected payoff warrants losing the stored effect. |
| Zhem | **Null Examiner** | Identify the enemy effect doing the most work, then cancel it; spend information to make selective silence more valuable than indiscriminate suppression. |
| Zhur | **Storm Surveyor** | Reveal lightning forecasts and hidden defenses before choosing a shock target; plan around Zhur's exemption from the Instability die without assuming every cast succeeds. |

### Vhorr — Husk-bearer

**Divine engine:** Seed recurring Hunger damage, let damaging ticks build pressure, and recover Breath from DoT kills.
...
| Khash | **Cinder Eater** | Establish burning victims and Hunger pressure, then choose whether to preserve an Aftertone for the long fight or consume it to finish a target. |
| Zhem | **Empty Vessel** | Cancel the buffs that sustain enemy resistance while weapon-seeded Hunger deals damage; silence weakens their setup without adding a damaging Zhem spell. |
| Zhur | **Storm Leech** | Use shock pressure to disrupt attempts to escape recurring Hunger; prioritize targets whose continued presence lets damaging ticks accumulate. |

### Vicoar — Flamebinder

**Divine engine:** Learn from fizzled casts; bank Failure Tokens and spend one to prevent a later cast from fizzling.
...
| Khash | **Flamebinder** | Attempt volatile fire bursts, bank the fizzles, then spend a token before consuming an Aftertone on the cast that must ignite. |
| Zhem | **Hushwright** | Learn from failed suppression and guarantee a later dispel; specialize in dismantling a dangerous enemy setup on the second attempt. |
| Zhur | **Sparkwright** | Use lightning's stable Instability behavior while banking any remaining fizzles; spend a token when a shock technique must land its casting check. |

### Ofshütje — Stormbearer

**Divine engine:** Choose a useful context for a hidden effect draw; adapt to the result rather than fully prescribing it.
...
| Khash | **Wildfire Herald** | Choose a fire-safe attack area with several useful outcomes; a hidden draw adds a secondary burning effect within that declared area, requiring fuel awareness. |
| Zhem | **Quiet Front** | Choose an enemy group with several active effects; a hidden draw removes an eligible buff or Aftertone, demanding follow-up based on what survives. |
| Zhur | **Stormbearer** | Commit lightning into a tactically useful target area; the hidden draw determines the storm's secondary payoff, while Zhur ignores the Instability die. |

### Pazzah — Oathclock

**Divine engine:** File consequences for a fixed future beat; control the intervening turns so they matter when due.
...
| Khash | **Ash Magistrate** | Schedule ignition over a chosen area, then herd enemies toward it or withdraw allies; the visible deadline makes movement part of the attack. |
| Zhem | **Oathclock** | File a suppression pulse for the turn an enemy setup becomes dangerous; cancel buffs and end Aftertones before they can deliver their planned payoff. |
| Zhur | **Storm Bailiff** | Schedule a shock effect at a contested position; coordinate the party's advance with its arrival rather than chasing an immediate damage opportunity. |

### Fickah — Locksmirk

**Divine engine:** Jam an enemy's pending action or queue; exploit the interruption while accepting the patron's fizzle floor.
...
| Khash | **Fuse Thief** | Jam the enemy's prepared answer before committing fire; consume an Aftertone for a burst during the opening and track the fuel you might ignite. |
| Zhem | **Hushjack** | Jam unresolved actions and use silence against buffs already in place; distinguish stopping a future effect from removing one that has already begun. |
| Zhur | **Spark Knave** | Use shock pressure to provoke an enemy commitment, then Jam its pending action; keep repositioning so its next attempt must solve a changed fight. |

### Izhakel — Threadwalker

**Divine engine:** Bind a condition to a target; make the opponent choose between obeying it and accepting the consequence.
...
| Khash | **Ember Notary** | Bind an enemy's next attack to ignition; place the contract where choosing aggression also risks nearby fuel and changes the surrounding terrain. |
| Zhem | **Silent Signatory** | Bind a target's next buffing action to the removal of an eligible existing buff or Aftertone; make strengthening its setup carry an explicit tradeoff. |
| Zhur | **Storm Notary** | Bind a movement or attack condition to shock; make the target choose between holding position, delaying aggression, or taking the storm's consequence. |

## Boundaries to resolve before implementation

| Question | Working direction for this chart |
|---|---|
| What do Scars do for Khor and Zhem builds? | They still guarantee an eligible mundane attack; the element supports or disrupts. New utility spends would require a separate design decision. |
| How do non-damaging Vhorr builds feed Hunger? | Ordinary attacks seed the existing class damage effect; Khor sustains the setup and Zhem strips opposition. Neither element becomes direct damage. |
| What does a non-damaging Attribution draw produce? | Eligible support extensions or removals rather than bonus damage. Every draw must help the declared plan; this is an extension of the current damage-only table. |
| Can Threads replace their damage payoff? | The proposal uses revelation, support, terrain and suppression payoffs. New conditions, eligibility, target limits and visibility must be authored before implementation. |
| How do delays and durations interact? | Pazzah controls when an effect starts; Khor and Vel control eligible duration after it starts. Duration extension must not postpone Ledger deadlines or multiply resource payouts. |

Utility pairings must not become weaker duplicates of damage pairings. Their first playtests
should check whether information, cover, duration and cancellation can decide encounters.
Every prototype needs costs, ranges, counters and readable tells; this chart intentionally
does not assign final numbers or rename current runtime classes.

## Sources and review

Grounded in the current [class resource matrix](../class-completion.md),
[class resource contract](../class-resources.md), [game identity rulings](../game-identity.md),
and [generated element definitions](../../data/generated/elements.json).
...
The companion [interactive chart](deity-element-class-chart.html) can filter by deity, element
or any word in a proposed name or playstyle. It is self-contained and needs no network access.
