...
| Jam | `request_cancel` does not affect holds; cancelling another working by the holder leaves upkeep running. | `test_hold_shroud_survives_a_jam_on_a_separate_deferred_working` |

The death-path case also commits a lethal Khash attack against an anchored, held Aftertone.
Aftertone writes captured before the kill cannot resurrect a released `held_by` flag.
...
- **CT upkeep:** “Upkeep timing for CT battles (measure vs turn). Implement turn start; note
  it.” Implemented at turn start; further CT recharge does not charge duplicate upkeep.
- **Jam:** “Whether a hold survives the holder being Jammed.” Preserved: it survives.
- **HUD:** held/anchored symbols and pointer selection of Notes remain outside this step.
