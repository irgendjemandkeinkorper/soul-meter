...
- ~~Ratified identity says XP comes from combat and quests, while current advancement says levels come from authored story milestones and never kill/use XP.~~ **RESOLVED 2026-09-08 (#285):** the owner re-affirmed ruling 9, kill-XP is in, and `Advancement` now derives levels from XP. Milestones survive as a large XP award rather than a second way to level. `docs/game-identity.md:37-38` `globals/advancement.gd`
