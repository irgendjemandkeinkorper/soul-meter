...
##
## SCHEMA ONLY — every FK here is empty in shipped data because this issue authors
## no jobs and no abilities.
##
...
## FK into `jobs` — the discipline the unit fields.
@export var primary_job_id: String = ""
## FK into `jobs` — the borrowed action set. Empty means no secondary.
